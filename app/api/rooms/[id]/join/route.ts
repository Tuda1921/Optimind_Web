...
//     const { id: roomId } = await params;
//     const body = await req.json();
//     const { password = "" } = body; // Default rỗng nếu không có
...
//     // --- FIX: Kiểm tra password ---
//     // Chỉ check nếu room thực sự có password
//     if (room.password && room.password.length > 0) {
//       if (room.password !== password) {
//         console.warn("Invalid password for room:", roomId);
//         return NextResponse.json({ error: "Mật khẩu phòng không đúng" }, { status: 403 });
//       }
//     }
...
//     if (existing) {
//       console.log("User already a member:", { roomId, userId: user.id });
//       return NextResponse.json({ error: "Already a member of this room", success: true, member: existing }, { status: 200 });
//     }
...
export async function POST(
  ...
  try {
    ...
    // --- FIX QUAN TRỌNG: Tự động dọn dẹp user khỏi các phòng khác ---
    // Trước khi cho vào phòng mới, xóa user khỏi TẤT CẢ các phòng hiện tại
    // Điều này giúp dọn dẹp các "bóng ma" (zombie) do lỗi crash trước đó
    ...
    // Join room (Không cần check existing nữa vì đã deleteMany ở trên)
    const member = await prisma.roomMember.create({
      data: {
        roomId,
        userId: user.id,
